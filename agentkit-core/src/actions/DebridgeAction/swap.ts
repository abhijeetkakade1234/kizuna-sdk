...
export async function smartSwap(
	...
	try {
		...
		try {
			const estimationUrl = formedDebridgeApiUrl.replace(
				"/transaction",
				"/estimation",
			);
...
3. You have enough balance of the input token`;
			...
			if (
				transactionData.errorMessage?.includes(
...
Example: "Swap 0.01 USDT to WETH with approveMax: true"`;
		...
		if (!transactionData.tx) {
			...
			const outSymbol =
				tokenOutDetails?.symbol || args.tokenOutSymbol || "tokens";
		...
		if (tokenInAddress !== "0x0000000000000000000000000000000000000000") {
			...
			if (approvalResult.userOpHash) {
				...
				console.log(
					`Token approval successful. UserOpHash: ${approvalResult.userOpHash}`,
				);
			}
		}
