...
export const SmartBridgeInput = z
	.object({
		...
		amount: z
			.string()
			.describe(
				"The amount of input token to bridge (human-readable, e.g., '100')",
			),
		recipientAddress: z
			.string()
			.startsWith("0x")
...
export async function smartBridge(
	...
	try {
		...
		if (tx.allowanceTarget && tx.allowanceValue) {
			...
			try {
				...
				if (!secondResponse.ok) {
					console.error(
						"Second Debridge API Error Response:",
					...
					const errorDetail =
						...
						JSON.stringify(
							secondApiResponseData.details || secondApiResponseData,
						);
				...
				console.log(
					"Successfully received final transaction data from second API call.",
				);
			} catch (error) {
				console.error(
					"Error calling Debridge API after preliminary approval:",
					error,
				);
		...
		const { recommendedSlippage, srcChainTokenIn, dstChainTokenOut } =
			estimation || {};
		...
		console.log("Submitting transaction to bridge:", {
			...
			dataLength: transactionToSubmit.data
				? transactionToSubmit.data.length
				: 0,
...
export class SmartBridgeAction
	implements AgentkitAction<typeof SmartBridgeInput>
{
