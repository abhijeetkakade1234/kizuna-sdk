...
export const DisperseInput = z
	...
	.strip()
	.describe(
		"Instructions for batch transferring tokens to multiple recipients",
	);
...
export async function disperseTokens(
	...
	try {
		...
		const tokenType = isEth
			? "ETH"
			: `tokens from contract ${args.tokenAddress}`;

		// Calculate total amount for summary
		const totalAmount = args.recipients.reduce((sum, recipient) => {
			return sum + Number.parseFloat(recipient.amount);
		}, 0);
		...
		const successfulTransfers: Array<{
			address: string;
			amount: string;
			txHash?: string;
		}> = [];
