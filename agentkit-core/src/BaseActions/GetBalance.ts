...
export const GetBalanceInput = z
	.object({
		tokenAddresses: z
			...
			.nullable()
			.describe(
				"Optional list of token contract addresses to get balances for",
			),
		tokenSymbols: z
			.array(z.string())
			.optional()
	...
	try {
		...
		if (
			...
			if (chainId && tokenMappings[chainId]) {
				...
				tokenAddresses = [
					...tokenAddresses,
					...Object.values(tokenMappings[chainId]),
				];
			} else {
				console.warn(
					`Chain ID ${chainId} not found in token mappings or is empty`,
				);
			}
		} else {
			// Process token addresses if provided
			if (args.tokenAddresses && args.tokenAddresses.length > 0) {
				tokenAddresses = args.tokenAddresses.map(
			...
			if (args.tokenSymbols && args.tokenSymbols.length > 0) {
				const symbolAddresses = await resolveTokenSymbols(
					wallet,
					args.tokenSymbols,
				);
		...
		const balanceStrings = balances
			...
			.filter((balance) => {
				...
				return (
					balance.formattedAmount !== "0" && balance.formattedAmount !== "0.0"
				);
			})
			.map((balance) => {
		...
		const responseTitle =
			...
			!args.tokenAddresses?.length &&
			!args.tokenSymbols?.length
				? "All Token Balances:"
				: "Balances:";
...
export class GetBalanceAction
	implements AgentkitAction<typeof GetBalanceInput>
{
