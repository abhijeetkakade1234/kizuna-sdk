...
// Helper for creating number schemas that are Gemini-compatible
const createNumberSchema = (min: number, max: number, description: string) =>
...
export const FormatUnitsSchema = z.object({
	value: z
		.string()
		.regex(/^-?\d+$/, "Value must be a string representing an integer (wei)."),
	decimals: createNumberSchema(
		1,
		256,
		"Number of decimal places (e.g., 18 for Ether, 6 for USDC)",
	),
});
...
export class FormatUnitsAction
	implements AgentkitAction<typeof FormatUnitsSchema>
{
...
export const ParseUnitsSchema = z.object({
	value: z
		...
		.regex(
			/^-?(\d+(\.\d+)?|\.\d+)$/,
			"Value must be a string representing a decimal number.",
		),
	decimals: createNumberSchema(
		1,
		256,
...
export class ParseUnitsAction
	implements AgentkitAction<typeof ParseUnitsSchema>
{
...
export class FormatEtherAction
	implements AgentkitAction<typeof FormatEtherSchema>
{
...
export class ParseEtherAction
	implements AgentkitAction<typeof ParseEtherSchema>
{
...
export const ToHexSchema = z.object({
	value: z
		...
		.describe(
			"The value to convert to Hex. Can be string, number, bigint, or boolean.",
		),
...
export const FromBytesSchema = z.object({
	...
	hexBytes: z
		...
		.refine(
			isHex,
			"Input must be a valid hex string representing bytes (0x...).",
		),
	to: z
		.enum(["string", "number", "bigint", "boolean"])
