...
export const ReadContractSchema = z.object({
	contractAddress: z
		.string()
		.refine(isAddress, "Invalid contract address format."),
	...
	functionName: z
		.string()
		.describe("The name of the view or pure function to call."),
...
export async function readContractFunc(
	...
	try {
		...
		if (!rpcProvider) {
			return formatError(
				"read_contract",
				"RPC Provider not found on the wallet instance.",
			);
		}
...
export class ReadContractAction
	implements AgentkitAction<typeof ReadContractSchema>
{
