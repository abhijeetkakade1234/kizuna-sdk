...
export async function sendTransaction(
	...
	if (useEoa()) {
		console.log("[sendTransaction] Using EOA mode");
		const eoa = getEoaFromEnv();
		if (!eoa) {
			console.error("[sendTransaction] EOA not configured");
			return {
				success: false,
	...
	try {
		console.log("[sendTransaction] Attempt 1: Sending via Smart Account with SPONSORED paymaster...");
		...
		console.log("[sendTransaction] Request response:", {
			hasError: !!request.error,
			error: request.error,
			hasUserOpHash: !!request.userOpHash,
		...
		if (request.error) {
			...
			if (errorStr.toLowerCase().includes('paymaster')) {
				console.warn("[sendTransaction] Paymaster error, falling back to self-funded transaction...");
				// Try without paymaster (smart account pays its own gas)
				return await sendSelfFundedTransaction(wallet, tx);
			}
			console.error("[sendTransaction] Request error:", request.error);
			return {
				success: false,
				error: errorStr,
			};
		...
		console.log("[sendTransaction] Waiting for receipt...");
		const receipt = await request.wait();
		console.log("[sendTransaction] Receipt received:", {
		...
		if (receipt.reason) {
			console.error("[sendTransaction] Receipt reason:", receipt.reason);
			return {
				success: false,
		...
		console.error("[sendTransaction] Exception caught:", error);
		const errorMessage = error instanceof Error ? error.message : String(error);
		
		// Check if it's a paymaster error and fall back
		if (errorMessage.toLowerCase().includes('paymaster')) {
			console.warn("[sendTransaction] Paymaster error detected, trying self-funded transaction...");
			return await sendSelfFundedTransaction(wallet, tx);
		}
		
		const errorStack = error instanceof Error ? error.stack : "";
		console.error("[sendTransaction] Error details:", { message: errorMessage, stack: errorStack });
		return {
			success: false,
...
/**
 * Send a transaction using the smart account's own funds (no paymaster)
 * This is a fallback when paymaster is not available
 */
async function sendSelfFundedTransaction(
	wallet: ZeroXgaslessSmartAccount,
	tx: Transaction,
): Promise<TransactionResponse> {
	try {
		console.log("[sendSelfFundedTransaction] Sending without paymaster (smart account pays gas)...");
		
		// Send without paymaster data - smart account will pay gas from its balance
		const request = await wallet.sendTransaction(tx);

		console.log("[sendSelfFundedTransaction] Request response:", {
			hasError: !!request.error,
			error: request.error,
			hasUserOpHash: !!request.userOpHash,
			userOpHash: request.userOpHash,
		...
		if (request.error) {
			console.error("[sendSelfFundedTransaction] Request error:", request.error);
			return {
				success: false,
				error: typeof request.error === 'string' ? request.error : JSON.stringify(request.error),
			};
		}

		console.log("[sendSelfFundedTransaction] Waiting for receipt...");
		const receipt = await request.wait();
		console.log("[sendSelfFundedTransaction] Receipt received:", {
			success: receipt.success,
			reason: receipt.reason,
			txHash: receipt.receipt?.transactionHash,
		});
		...
		if (receipt.reason) {
			console.error("[sendSelfFundedTransaction] Receipt reason:", receipt.reason);
			return {
				success: false,
				error: receipt.reason,
			};
		...
		if (receipt.success && receipt.receipt?.transactionHash) {
			return {
				success: true,
				txHash: receipt.receipt?.transactionHash,
				message: `Transaction confirmed (self-funded)!\nTx Hash: ${receipt.receipt?.transactionHash}\n\n.`,
				receipt,
			};
		}
		return {
			success: false,
			error: `Transaction failed: ${receipt.reason || "Unknown reason"}`,
		};
	} catch (error) {
		console.error("[sendSelfFundedTransaction] Exception caught:", error);
		const errorMessage = error instanceof Error ? error.message : String(error);
		return {
			success: false,
			error: `Self-funded Transaction Error: ${errorMessage}`,
		};
	}
}
...
export async function getWalletBalance(
	...
	try {
		...
		console.error("wallet.getBalances failed, falling back to native balance:", error);
	}

	// Fallback: get native balance using rpcProvider
	try {
		const address = await wallet.getAddress();
		const nativeBalance = await wallet.rpcProvider.getBalance({
			address: address as `0x${string}`
		});
		return [
			{
				address: "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",
				formattedAmount: formatEther(nativeBalance),
			} as any
		];
	} catch (fallbackError) {
		console.error("Balance fallback also failed:", fallbackError);
		return false;
	}
}
